...
if config.get("read_contig") == None and config.get("should_install_genomad") == None:
    print("ERROR: read_contig not passed to snakemake as config. Define either. Eg. snakemake <arguments> --config read_contig=<read file>. If in doubt refer to the README.md file")
    sys.exit()
...
sample_id_path_contigs = collections.defaultdict(dict)
for id, (read1, read2, contigs) in enumerate(zip( df.read1, df.read2, df.contigs)):
    #id = f"sample{str(id)}"
    id = f"{str(id)}"
    sample = "intermidiate_files"
    sample_id[sample].append(id)
    sample_id_path[sample][id] = [read1, read2]
    sample_id_path_contigs[sample][id] = [assembly]
