...
1. Download the input files, reads, and assembly directories.
    1. `wget https://www.erda.dk/archives/b3aefff8c456500c3ac384d6a6199f08/published-archive.html; cd CAMI2_assemblies_and_PlasMAAG_input # download assembly directories`
    2. `wget https://erda.ku.dk/archives/826fe4d8889f88db2ec20058f9eaa015/published-archive.html; tar -xvf reassembled_CAMI_reads.tar.gz # download reads directories and decompress them `
...
6. Once we get the cluster files. We can evaluate the binning with BinBencher. Here [https://viralinstruction.com/BinBencherBackend.jl/dev/walkthrough/] is an extensive documentation about how to run BinBencher. In short, after installing julia and BinBencher, we just have to:
    1. `wget https://www.erda.dk/archives/426ef81eb35ea07078bb0041ee186c84/published-archive.html # Download references`
...
8. Evaluate SCAPP binning with BinBencher
    1. `julia # Start and interactive julia session`
        
        ```julia
        using BinBencherBackend
        ref_f =  “references/scapp_references/Airways.json”
...
1. Download input files, reads, and assembly directories.
    1. `wget https://www.erda.dk/archives/571769227931c6c4325a284f18e2bf54/published-archive.html ; cd DARWIN_assemblies_reads_and_PlasMAAG_input # download assembly, reads, and PlasMAAG input file`
2. Follow instructions to install PlasMAAG from zenodo [https://zenodo.org/records/17953597] 
3. Run PlasMAAG:
    1. `conda activate PlasMAAG_zenodo # activate environment`
    2. `PlasMAAG --reads_and_assembly_dir read_and_assembly_dir.tsv  --output test_run_DARWIN -t 16 --vamb_arguments '-o C --seed 1’ --genomad_thr '0.95’  `
...
5. Once we get the cluster files. We can evaluate the binning with BinBencher. Here [https://viralinstruction.com/BinBencherBackend.jl/dev/walkthrough/] is an extensive documentation about how to run BinBencher. In short, after installing julia and BinBencher, we just have to:
    1. `wget https://www.erda.dk/archives/426ef81eb35ea07078bb0041ee186c84/published-archive.html # Download references, only required if you did not do it for Figure 2.`
    2. `julia # Start and interactive julia session`
...
7. Evaluate SCAPP binning with BinBencher
    ...
    2. `julia # Start and interactive julia session`
        ...
        ```julia
        using BinBencherBackend
        ref_f =  “references/scapp_refernces/DARWIN_long_reads.json”
